...
public class CircusWorld implements World{
    ...
    private static int NUM_OF_CLOWNS;
    ...
    public CircusWorld(int width, int height){
        this.width = width;
        this.height = height;
    ...
    private void createGameObject(){
        setLevel(new LevelContext(new Level1()));
        constant.add(new ImageObject(0, 0, "background.png"));
        for(int i = 0;i<5;i++)
            constant.add(new ImageObject(60*i, 0, "heart.png"));
        spawn();
        for(int i=0;i<NUM_OF_CLOWNS;i++)
            control.add(new ClownObject(width/4*i, (int)(height*0.515),"clown.png"));
    }
    ...
    private void setLevel(LevelContext con){
        ...
        NUM_OF_CLOWNS = con.setNoofClowns();
    }
...
//            }
    ...
    public String getStatus() {
        return "Score: "+score+ "    |    HighScore: "+highscore+"   |    Time="+ Math.max(0, (MAX_TIME-(System.currentTimeMillis()-startTime))/1000);
    }
